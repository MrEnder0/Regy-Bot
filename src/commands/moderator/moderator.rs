...
pub async fn moderator(
    ...
    match args[0] {
        ...
        "add_infraction" => {
            ...
            if user_id == "none" {
                ctx.say("You need to specify a user id.").await.log_expect("Unable to send message");
                return Ok(());
            }
            let user_id = user_id.parse::<u64>().unwrap();
            toml::add_infraction(user_id);
            ctx.say("Added infraction to the specified user.").await.log_expect("Unable to send message");
            Ok(())
        }
        "remove_infraction" => {
            let user_id = arg.split_whitespace().nth(1).unwrap_or("none");
            ...
            ctx.say("Removed 1 infraction from the specified user.").await.log_expect("Unable to send message");
            Ok(())
        }
        "list_infractions" => {
            let user_id = arg.split_whitespace().nth(1).unwrap_or("none");
            ...
            ctx.say(formatted_infractions).await.log_expect("Unable to send message");
            Ok(())
        }
        "grab_pfp" => {
            let user_id = arg.split_whitespace().nth(1).unwrap_or("none");
            ...
            ctx.say(user.face()).await.log_expect("Unable to send message");
            Ok(())
        }
        "grab_banner" => {
            ...
            let user_id = user_id.parse::<u64>().unwrap();
            let user = UserId(user_id).to_user(ctx).await?;
            ctx.say(user.banner_url().unwrap_or("This user does not have a banner!".to_string())).await.log_expect("Unable to send message");
            Ok(())
        }
        "am_mod" => {
            ctx.say("Yes, now do some moderation and stop making me do it. :|").await.log_expect("Unable to send message");
            Ok(())
        }
        _ => {
