...
pub async fn user(
    ...
    match args[0] {
        ...
        "am_user" => match ctx.author().id.as_u64() {
            687897073047306270 => {
                ctx.say("Why would you not be a user you skid :skull: \nOh wait... You're Ender :sweat_smile:... Sorry I still love u, you're not a skid. :heart: :hot_face:").await.log_expect("Unable to send message");
                Ok(())
            }
            598280691066732564 => {
