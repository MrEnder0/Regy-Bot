...
pub fn gen_server(guid_id: String, log_channel: u64) {
    ...
    config.servers.insert(guild_id, ServerOptions {
    ...
    log_this(LogData {
        importance: LogImportance::Info,
        message: format!("A server with the id {} has been added to the config file.", guid_id),
    });
}
...
pub fn add_regex (server_id: String, phrase: String) -> bool {
    ...
    config.servers.get_mut(&server_id).unwrap().block_phrases.insert(Uuid::new_v4().to_string(), general_purpose::STANDARD_NO_PAD.encode(&phrase));
    let toml = toml::to_string(&config).unwrap();
    std::fs::write("config.toml", toml).unwrap();
...
pub fn remove_regex (server_id: String, id: Uuid) -> bool {
    ...
    config.servers.get_mut(&server_id).unwrap().block_phrases.remove(&id.to_string());
    let toml = toml::to_string(&config).unwrap();
    std::fs::write("config.toml", toml).unwrap();
...
pub fn add_infraction(server_id: String, id: u64) -> bool {
    ...
    let infractions = config.servers.get_mut(&server_id).unwrap().infractions.entry(id.to_string()).or_insert(0);
    *infractions += 1;
    let toml = toml::to_string(&config).unwrap();
    std::fs::write("config.toml", toml).unwrap();
...
pub fn dismiss_infraction(server_id: String, id: u64) -> bool {
    ...
    let infractions = config.servers.get_mut(&server_id).unwrap().infractions.entry(id.to_string()).or_insert(1);
    if *infractions <= 0 {
        return false
    } else {
        *infractions -= 1;
...
pub fn remove_staff(server_id: String, id: u64) -> bool {
    ...
    if config.servers.get(&server_id).unwrap().staff.contains(&id) {
        let user_index = config.servers.get(&server_id).unwrap().staff.iter().position(|x| *x == id).unwrap();
        config.servers.get_mut(&server_id).unwrap().staff.remove(user_index);
        let toml = toml::to_string(&config).unwrap();
        std::fs::write("config.toml", toml).unwrap();
...
pub fn delete_user(server_id: String, id: u64) {
    ...
    if config.servers.get(&server_id).unwrap().infractions.contains_key(&id.to_string()) {
        config.servers.get_mut(&server_id).unwrap().infractions.remove(&id.to_string());
    }

    //Checks if user is on staff list and removes them if they are
    if config.servers.get(&server_id).unwrap().staff.iter().any(|x| *x == id) {
